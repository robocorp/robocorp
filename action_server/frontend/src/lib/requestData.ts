...
class ModelUpdater {
  ...
  constructor(modelContainer: ModelContainer) {
    ...
    this.sio = io(baseUrlWs, { path: '/api/ws' });
    this.sio.on('echo', (echoVal) => {
      console.log('echo val was', echoVal);
    });
    this.sio.emit('echo', 'somevalue');

    this.sio.on('runs_collected', (data) => {
      const { runs } = data;
  ...
  public async startUpdating() {
    ...
    // Start listening for run events.
    this.sio.on('connect', () => {
      // If we disconnect, on a new connection we have to start listening again.
