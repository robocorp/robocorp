...
def _fix_contents_version_in_poetry(contents, version):
    ...
    contents = re.sub(
        r"(robocorp-robolog\s*=\s*)\"\^?\d+\.\d+\.\d+", r'\1"%s' % (version,), contents
    )
    return contents
...
class Dev(object):
    def set_version(self, version):
        """
        Sets a new version for robocorp-robolog in all the needed files.
        """
    ...
    def get_tag(self):
        ...
        cmd = "git describe --tags --abbrev=0 --match robocorp-robolog*".split()
        popen = subprocess.Popen(cmd, stdout=subprocess.PIPE)
        stdout, stderr = popen.communicate()

        # Something as: b'robocorp-robolog-0.0.1'
    ...
    def check_tag_version(self):
        ...
        if robolog.__version__ == version:
            sys.stderr.write("Version matches (%s) (exit(0))\n" % (version,))
            sys.exit(0)
        else:
            sys.stderr.write(
                "Version does not match (robocorp-robolog: %s != repo tag: %s).\nTags:%s\n(exit(1))\n"
                % (robolog.__version__, version, self.get_all_tags())
            )
            sys.exit(1)
...
# output generated by robocorp-robolog (i.e.: the .robolog files).
