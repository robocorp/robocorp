def test_errors(log_setup, tmpdir):
    ...
    from robocorp.robolog import verify_log_messages_from_stream
    from robocorp.robolog.protocols import Status
    ...
    robolog.start_run("My Run")
    robolog.start_task("task", "modname", str(tmpdir), 0, [])

    try:
        check_traceback.main()
    except RuntimeError as e:
        robolog.end_task("task", "modname", Status.ERROR, str(e))
        robolog.end_run("My Run", Status.ERROR)
    else:
