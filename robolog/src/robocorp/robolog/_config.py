...
_ROBO_LOG_MODULE_NAME = robolog.__name__
...
class ConfigFilesFiltering(BaseConfig):
    ...
    def _get_modname_or_file_filter_kind(
        ...
        absolute_filename = robolog._files_filtering._absolute_normalized_path(filename)
        ...
        exclude = not robolog._in_project_roots(absolute_filename)
        if exclude:
