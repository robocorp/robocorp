...
def test_keyword_read_table_from_csv(library):
    table = library.read_table_from_csv(RESOURCES_DIR / "easy.csv")
    assert len(table) == 3
...
def test_keyword_read_table_from_csv_no_header(library):
    table = library.read_table_from_csv(RESOURCES_DIR / "easy.csv", header=False)
    assert len(table) == 4
...
def test_keyword_read_table_from_csv_dialect_string(library):
    table = library.read_table_from_csv(
        RESOURCES_DIR / "hard.csv", dialect="excel", header=True
    )
    assert len(table) == 100


def test_keyword_read_table_from_csv_encoding(library):
    table = library.read_table_from_csv(RESOURCES_DIR / "easy.csv", encoding="utf-8")
    assert len(table) == 3
    assert table.columns == ["first", "second", "third"]
    assert table[0] == ["1", "2", "3"]
...
def test_keyword_read_table_from_csv_longer_lines(library):
    table = library.read_table_from_csv(
        RESOURCES_DIR / "big.csv", header=False, delimiters=";"
    )
    assert len(table) == 3
...
def test_keyword_read_table_from_csv_extra(library):
    table = library.read_table_from_csv(
        RESOURCES_DIR / "extra.csv", column_unknown="whoknows"
    )
    assert len(table) == 4
...
def test_keyword_read_table_from_csv_manual(library):
    table = library.read_table_from_csv(
        RESOURCES_DIR / "hard.csv", dialect=Dialect.Excel, header=True
    )
    assert len(table) == 100
    assert table.columns == [
        "Region",
