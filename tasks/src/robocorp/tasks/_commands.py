...
def serve(
    ...
    from ._log_output_setup import setup_log_output, setup_log_output_to_port
    from ._protocols import ITask, Status
    from ._server import QueueResponse, run_server
    from ._task import Context, set_current_task
        ...
        try:
            ...
            try:
                ...
                for req in requests:
                    ...
                    if not matches:
                        err = f"Requested unknown task: {req.name}"
                        context.show_error(err)
                        response = QueueResponse(status=False, error=err, value=None)
                        requests.send_response(response)
                        continue
                    ...
                    before_task_run(task)
                    try:
                        result = task.run(**req.fields)
                        run_status = task.status = Status.PASS
                        response = QueueResponse(status=True, error=None, value=result)
                        requests.send_response(response)
                    except Exception as e:
                        run_status = task.status = Status.ERROR
                        if not no_status_rc:
                        ...
                        response = QueueResponse(
                            status=False, error=task.message, value=None
                        )
                        requests.send_response(response)
                    finally:
                        after_task_run(task)
                ...
                context.show_error("User interrupt, closing gracefully")
            finally:
                log.start_task("Teardown tasks", "teardown", "", 0)
                try:
                    after_all_tasks_run(tasks)
